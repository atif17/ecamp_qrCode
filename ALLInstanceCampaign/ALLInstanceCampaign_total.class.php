...
class ALLInstanceCampaign_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['ALLInstanceCampaign']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['ALLInstanceCampaign']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['tot_geral'] = array() ;  
      $nm_comando = "select count(*) from (SELECT     RecID,     CampaignName,     Discription,     Status,     Mode,     SmtpProfile,     SmtpServer,     UserName,     Password,     ApiProfile,     SendTo,     ToContact,     MsgType,     Subject,     Message,     FromMail,     FromName,     RecurringDuration,     CreatedBy,     CreatedDate,     ModifiedBy,     ModifiedTime,     ToEmail,     Template,     UploadFile,     EmailSend,     EmailCount,     StartDate,     StartTime,     RecurringType, (select sum(BounceEmailStatus) from emailupload where emailupload.FileName = emailcampaign.ToContact collate utf8_unicode_ci) AS BounceEmailNo FROM     emailcampaign where (Status = 'Running' or Status = 'Paused')) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['contr_total_geral'] = "OK";
   } 
