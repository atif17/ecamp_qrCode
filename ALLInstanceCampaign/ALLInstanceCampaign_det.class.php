...
class ALLInstanceCampaign_det
{
 ...
 function monta_det()
 {
    ...
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['embutida_form']) && $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['embutida_form'])
    {
    $this->nmgp_botoes['det_pdf']   = "off";
    $this->nmgp_botoes['det_print'] = "off";
    }
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['campos_busca']))
    { 
        $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['campos_busca'];
...
$_SESSION['scriptcase']['ALLInstanceCampaign']['contr_erro'] = 'off'; 
    ...
    {   
        $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['where_pesq'] .= $this->nm_where_dinamico;
    }   
    $this->NM_raiz_img  = ""; 
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['seq_dir'] = 0; 
    $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['sub_dir'] = array(); 
   ...
   { 
       $nmgp_select = "SELECT CampaignName, Discription, Status, str_replace (convert(char(10),StartDate,102), '.', '-') + ' ' + convert(char(8),StartDate,20), SmtpProfile, Template, BounceEmailNo, RecID, ToContact, EmailCount from (SELECT     RecID,     CampaignName,     Discription,     Status,     Mode,     SmtpProfile,     SmtpServer,     UserName,     Password,     ApiProfile,     SendTo,     ToContact,     MsgType,     Subject,     Message,     FromMail,     FromName,     RecurringDuration,     CreatedBy,     CreatedDate,     ModifiedBy,     ModifiedTime,     ToEmail,     Template,     UploadFile,     EmailSend,     EmailCount,     StartDate,     StartTime,     RecurringType, (select sum(BounceEmailStatus) from emailupload where emailupload.FileName = emailcampaign.ToContact collate utf8_unicode_ci) AS BounceEmailNo FROM     emailcampaign where (Status = 'Running' or Status = 'Paused')) nm_sel_esp"; 
   } 
   else 
   { 
       $nmgp_select = "SELECT CampaignName, Discription, Status, StartDate, SmtpProfile, Template, BounceEmailNo, RecID, ToContact, EmailCount from (SELECT     RecID,     CampaignName,     Discription,     Status,     Mode,     SmtpProfile,     SmtpServer,     UserName,     Password,     ApiProfile,     SendTo,     ToContact,     MsgType,     Subject,     Message,     FromMail,     FromName,     RecurringDuration,     CreatedBy,     CreatedDate,     ModifiedBy,     ModifiedTime,     ToEmail,     Template,     UploadFile,     EmailSend,     EmailCount,     StartDate,     StartTime,     RecurringType, (select sum(BounceEmailStatus) from emailupload where emailupload.FileName = emailcampaign.ToContact collate utf8_unicode_ci) AS BounceEmailNo FROM     emailcampaign where (Status = 'Running' or Status = 'Paused')) nm_sel_esp"; 
   } 
   $parms_det = explode("*PDet*", $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['chave_det']) ; 
   ...
   {
       $nmgp_select .= " where  RecID = $parms_det[0]" ;  
   } 
   else 
   { 
       $nmgp_select .= " where  RecID = $parms_det[0]" ;  
   ...
   $this->discription = $rs->fields[1] ;  
   $this->status = $rs->fields[2] ;  
   $this->startdate = $rs->fields[3] ;  
   $this->smtpprofile = $rs->fields[4] ;  
   $this->template = $rs->fields[5] ;  
   $this->bounceemailno = $rs->fields[6] ;  
   $this->bounceemailno = (string)$this->bounceemailno;
   $this->recid = $rs->fields[7] ;  
   $this->recid = (string)$this->recid;
   $this->tocontact = $rs->fields[8] ;  
   $this->emailcount = $rs->fields[9] ;  
...
//--- 
   ...
   if ($_SESSION['scriptcase']['proc_mobile'])
   {
       $nm_saida->saida(" <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0\" />\r\n");
           ...
           $nm_saida->saida("   <script type=\"text/javascript\" src=\"ALLInstanceCampaign_jquery-3.6.0.min.js\"></script>\r\n");
           $nm_saida->saida("   <script type=\"text/javascript\" src=\"ALLInstanceCampaign_ajax.js\"></script>\r\n");
           $nm_saida->saida("   <script type=\"text/javascript\" src=\"ALLInstanceCampaign_message.js\"></script>\r\n");
...
$nm_saida->saida("</script>\r\n");
   ...
   if (($this->Ini->sc_export_ajax || $this->Ini->Export_det_zip) && $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['det_print'] == "print")
   {
       ...
       $NM_css_cmp  = $this->Ini->path_link . "ALLInstanceCampaign/ALLInstanceCampaign_det_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css";
       $nm_saida->saida("  <style type=\"text/css\">\r\n");
   ...
   elseif (($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['det_print'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
   {
       ...
       $nm_saida->saida(" <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "ALLInstanceCampaign/ALLInstanceCampaign_det_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
   }
   else
   {
   ...
   if (!$_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['pdf_det'] && $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['det_print'] != "print")
   {
   ...
   $nm_saida->saida("</HEAD>\r\n");
    $removeMargin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['link_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['link_info']['remove_margin'] ? 'margin: 0;' : '';
    $removeBorder = isset($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['link_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['link_info']['remove_border'] ? 'border-width: 0' : '';
   if (!$this->Ini->Export_html_zip && !$this->Ini->Export_det_zip && $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['det_print'] == "print")
   {
       $nm_saida->saida(" <link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/buttons/" . $this->Ini->Str_btn_css . "\" /> \r\n");
...
$_SESSION['scriptcase']['ALLInstanceCampaign']['contr_erro'] = 'off'; 
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['cmp_acum']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['cmp_acum']))
   {
       $parms_acum = explode(";", $_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['cmp_acum']);
   ...
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->campaignname)));
       $conteudo_original =  NM_encode_input(sc_strip_script($conteudo)); 
   }
   else {
       $conteudo = trim(sc_strip_script($this->campaignname)); 
   ...
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_campaignname_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_campaignname_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_campaignname\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf")
   ...
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->discription)));
       $conteudo_original =  NM_encode_input(sc_strip_script($conteudo)); 
   }
   else {
       $conteudo = trim(sc_strip_script($this->discription)); 
       $conteudo_original = $conteudo; 
   }
          if ($conteudo === "") 
          { 
          ...
          $str_tem_display = $conteudo;
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_discription_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_discription_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_discription\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf")
   ...
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_status_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_status\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf")
   ...
   $conteudo = trim(NM_encode_input(sc_strip_script($this->startdate))); 
   $conteudo_original = $conteudo; 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          ...
          $str_tem_display = $conteudo;
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_startdate_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_startdate_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_startdate\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf")
   ...
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->smtpprofile)));
       $conteudo_original =  NM_encode_input(sc_strip_script($conteudo)); 
   }
   else {
       $conteudo = trim(sc_strip_script($this->smtpprofile)); 
       $conteudo_original = $conteudo; 
   }
          if ($conteudo === "") 
          { 
          ...
          $str_tem_display = $conteudo;
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_smtpprofile_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_smtpprofile_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_smtpprofile\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf")
   ...
   $SC_Label = (isset($this->New_label['template'])) ? $this->New_label['template'] : "Template"; 
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->template)));
       $conteudo_original =  NM_encode_input(sc_strip_script($conteudo)); 
   }
   else {
       $conteudo = trim(sc_strip_script($this->template)); 
       $conteudo_original = $conteudo; 
   }
          if ($conteudo === "") 
          { 
          ...
          $str_tem_display = $conteudo;
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_template_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_template_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_template\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['opcao'] == "pdf")
   ...
   $conteudo = trim(NM_encode_input(sc_strip_script($this->bounceemailno))); 
   $conteudo_original = $conteudo; 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          ...
          $str_tem_display = $conteudo;
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_bounceemailno_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_bounceemailno_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_bounceemailno\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("</TABLE>\r\n");
   if(isset($_SESSION['scriptcase']['proc_mobile']) && $_SESSION['scriptcase']['proc_mobile'])
...
//--- 
   ...
   $nm_saida->saida("                  action=\"ALLInstanceCampaign_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("       NovaJanela = window.open (\"ALLInstanceCampaign_doc.php?nmgp_parms=\" + campo1, \"ScriptCase\", \"resizable\");\r\n");
   $nm_saida->saida("   }\r\n");
   ...
   {
      global $nm_saida;
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['det_print'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['ALLInstanceCampaign']['pdf_det']) 
   { 
       ...
       if ($this->nmgp_botoes['det_pdf'] == "on")
       {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Dpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "ALLInstanceCampaign/ALLInstanceCampaign_config_pdf.php?nm_opc=pdf_det&nm_target=0&nm_cor=cor&papel=8&orientacao=1&largura=1200&conf_larg=S&conf_fonte=10&language=en_us&conf_socor=N&sc_ver_93=s&password=n&pdf_zip=N&KeepThis=false&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
       if ($this->nmgp_botoes['det_print'] == "on")
       {
         $this->nm_btn_exist['det_print'][] = "Dprint_top";
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Dprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "ALLInstanceCampaign/ALLInstanceCampaign_config_print.php?nm_opc=detalhe&nm_cor=CO&password=n&language=en_us&KeepThis=true&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
